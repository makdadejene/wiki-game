...
let find_friend_group network ~person : Person.t list =
  let graph =
    Set.fold
      ...
      Map.update init person1 ~f:(fun nbors ->
        match nbors with
        | Some list -> list @ [ person2 ]
        | None -> [ person2 ]))
  in
  let visited = String.Hash_set.create () in
  let not_visited = Queue.create () in
  Queue.enqueue not_visited person;
  let rec traverse () =
    match Queue.dequeue not_visited with
    | None -> ()
    | Some curr_node ->
      if not (Hash_set.mem visited curr_node)
      then (
        Hash_set.add visited curr_node;
        let adjacent_nodes = Map.find_exn graph curr_node in
        List.iter adjacent_nodes ~f:(fun next ->
          Queue.enqueue not_visited next);
        traverse ())
      else traverse ()
  in
  traverse ();
